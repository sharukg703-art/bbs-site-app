      ...
      <div className="px-6 pb-10">
        <div className="bbs-card p-6 md:p-8 w-full max-w-sm mx-auto">
          ...
          <p className="text-xs mt-2 leading-relaxed" style={{ color: "#F2C14E" }}>
            Site Records and Daily Progress Reports are SHARED — anyone who opens this app sees the same
            site-wide log. Don't put confidential data in here.
          </p>
        </div>
      </div>
...
export default function BBSSiteApp() {
  ...
  const shareFile = async () => {
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        ...
        setSaveMsg("File sharing isn't supported on this browser/device — use the download link instead.");
        setTimeout(() => setSaveMsg(""), 3500);
      }
    } catch (e) {
      if (e && e.name !== "AbortError") {
        console.error("Share failed:", e);
        setSaveMsg("Share failed — use the download link instead.");
  ...
  const SECTIONS = [
    ...
    { id: "records", label: `${t("records")} (${records.length})`, icon: FolderOpen },
    { id: "dpr", label: `${t("dpr")} (${dprEntries.length})`, icon: ClipboardList },
  ];
  ...
  return (
    <div className="bbs-root p-4 md:p-6">
      ...
      {downloadReady && (
        <div className="bbs-card p-4 mb-4" style={{ borderColor: "#7FD6A6", borderWidth: 2 }}>
          ...
          <div className="text-xs mt-2" style={{ color: "#BFE0F5" }}>
            If tapping the filename link doesn't save automatically, press and hold it, then choose "Download link" or
            "Save link as." Or tap <strong>Share</strong> to send it straight to WhatsApp, Gmail, Drive, etc. — sharing
            often works even when direct downloads are blocked.
          </div>
      ...
      {/* Section nav */}
      <div className="flex flex-wrap gap-2 mb-5 bbs-no-print">
        {SECTIONS.map((s) => {
          const Icon = s.icon;
          return (
