...
export default function BBSSiteApp() {
  ...
  const handlePrint = () => {
    ...
    setPrintData({ title: "All Members - Current Calculation", meta, rows: allRows, total: grandTotal });
  ...
  const printRecord = (rec) => {
    ...
    setPrintData({ title: `${rec.memberType} - ${rec.label}`, meta, rows: rec.rows, total: rec.totalWeight });
    setTimeout(() => window.print(), 80);
  };
  ...
  const editDpr = (entry) => { setDprForm({ date: entry.date, work: entry.work, manpower: entry.manpower, material: entry.material, weather: entry.weather, remarks: entry.remarks, photo: entry.photo || null }); setEditingDprId(entry.id); };
    ...
    try {
      ...
      if (myDprEntries.length) {
        const dprAoa = [["DAILY PROGRESS REPORTS"], [], ["Date", "Work Carried Out", "Manpower", "Material Used", "Weather", "Remarks"],
          ...myDprEntries.map((e) => [e.date, e.work, e.manpower, e.material, e.weather, e.remarks])];
      ...
      {section === "dpr" && (
        <div>
          ...
          <div className="bbs-card p-4">
            <h3 className="bbs-display text-base font-bold mb-3">Log ({myDprEntries.length})</h3>
            {myDprEntries.length === 0 && (<div className="text-sm bbs-mono" style={{ color: "#6E9BBE" }}>No entries yet.</div>)}
            <div className="space-y-2">
              {myDprEntries.map((e) => (
